...
/*   Updated: 2023/03/22 16:50:33 by mbocquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parsing_map(t_param *prm, char *file_name)
{
	...
	prm->map.map[4] = ft_strdup_gc(prm, 0, "100000110110000111110001");
	...
	prm->map.map[8] = ft_strdup_gc(prm, 0, "100000110110000111110001");
	prm->map.map[9] = ft_strdup_gc(prm, 0, "100000000000000000000001");
	prm->map.map[10] = ft_strdup_gc(prm, 0, "100000000000000000000001");
	prm->map.map[11] = ft_strdup_gc(prm, 0, "111100111000000000000001");
	prm->map.map[12] = ft_strdup_gc(prm, 0, "110100001000000000000001");
	prm->map.map[13] = ft_strdup_gc(prm, 0, "110000001000000000000001");
	prm->map.map[14] = ft_strdup_gc(prm, 0, "110100000000000011000001");
	prm->map.map[15] = ft_strdup_gc(prm, 0, "110111111000000011000001");
	prm->map.map[16] = ft_strdup_gc(prm, 0, "110000000000000000000001");
	prm->map.map[17] = ft_strdup_gc(prm, 0, "111111111000000000000001");
	prm->map.map[18] = ft_strdup_gc(prm, 0, "110000000000000000000001");
	prm->map.map[19] = ft_strdup_gc(prm, 0, "111111111111111111111111");
	if (get_nb_str(prm->map.map) != 20)
		return (1);
	return (0);
}
...
/*int	parsing_map(t_param *prm, char *file_name)
{
	...
	prm->map.map[18] = ft_strdup_gc(prm, 0, "110100001000000011000001");
	prm->map.map[19] = ft_strdup_gc(prm, 0, "110111111000000011000001");
