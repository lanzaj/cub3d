...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/04 20:21:35 by mbocquel          #+#    #+#             */
/*   Updated: 2023/04/04 20:21:43 by mbocquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	is_first_line_only_wall(t_param *prm, char **map)
{
	...
	while (map[0][i])
	{
		if (!ft_strchr("13456789", map[0][i]) && map[0][i] != '2')
...
static void	is_first_column_only_wall(t_param *prm, char **map)
{
	...
	while (map[i])
	{
		if (!ft_strchr("13456789", map[i][0]) && map[i][0] != '2')
			check_map_error(prm, "Error\nNot enclosed in walls\n");
		i++;
	}
}
