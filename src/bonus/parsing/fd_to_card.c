...
static void	fd_to_card_part4(t_param *prm, int fd, char *str)
{
	...
	if (!ft_strncmp(str, "D1 ", 3) && prm->map.die_texture[1].img == NULL
		&& import_img(prm, &(prm->map.die_texture[1]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid die1 texture\n");
	if (!ft_strncmp(str, "D2 ", 3) && prm->map.die_texture[2].img == NULL
		&& import_img(prm, &(prm->map.die_texture[2]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid die2 texture\n");
	if (!ft_strncmp(str, "D3 ", 3) && prm->map.die_texture[3].img == NULL
		&& import_img(prm, &(prm->map.die_texture[3]), &str[3]))
...
static void	fd_to_card_part3(t_param *prm, int fd, char *str)
{
	...
	if (!ft_strncmp(str, "F0 ", 3) && prm->map.front_texture[0].img == NULL
		&& import_img(prm, &(prm->map.front_texture[0]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid front0 texture\n");
	if (!ft_strncmp(str, "F1 ", 3) && prm->map.front_texture[1].img == NULL
		&& import_img(prm, &(prm->map.front_texture[1]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid front1 texture\n");
	if (!ft_strncmp(str, "F2 ", 3) && prm->map.front_texture[2].img == NULL
		&& import_img(prm, &(prm->map.front_texture[2]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid front2 texture\n");
	if (!ft_strncmp(str, "F3 ", 3) && prm->map.front_texture[3].img == NULL
		&& import_img(prm, &(prm->map.front_texture[3]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid front3 texture\n");
	if (!ft_strncmp(str, "A0 ", 3) && prm->map.attack_texture[0].img == NULL
		&& import_img(prm, &(prm->map.attack_texture[0]), &str[3]))
		fd_to_map_error(prm, fd, "Error\nInvalid attack0 texture\n");
	if (!ft_strncmp(str, "A1 ", 3) && prm->map.attack_texture[1].img == NULL
		&& import_img(prm, &(prm->map.attack_texture[1]), &str[3]))
